 ...
 * oorspronkelijke nodes-import plaatsvond vóór de deterministische-ID-fix —
 * een latere herhaalde import creëerde daardoor 13.152 extra documenten
 * naast de originele set, in plaats van ze te overschrijven.
 *
 * dryRun=1 (default): toont alleen wat verwijderd zou worden.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const snapshot = await db.collection("sourceNodes").where("datasetVersionId", "==", datasetVersionId).get();
    ...
    if (!dryRun) {
      ...
      for (let i = 0; i < toDelete.length; i += BATCH_LIMIT) {
        ...
        for (const id of chunk) {
          batch.delete(db.collection("sourceNodes").doc(id));
        }
        await batch.commit();
      }
    }
    ...
    return NextResponse.json({
      datasetVersionId,
      dryRun,
      totalSourceNodeDocs: snapshot.size,
      emptySourceObjectIdCount,
